"""Generated primitive_array model: ocsf_account_labels.
...
class OcsfOcsfAccountLabels(OcsfBase):
    """Association table for account.labels.

    Stores primitive array values in a normalized one-to-many relationship.
    """
    __tablename__ = "ocsf_account_labels"
    ...
    value: Mapped[Optional[str]] = mapped_column(
        Text,
        nullable=True,
        comment="Values for account.labels",
    )
    ...
    account: Mapped["OcsfAccount"] = relationship(
        "OcsfAccount",
        back_populates="labels",
    )

    def __repr__(self) -> str:
        return f"<OcsfOcsfAccountLabels(id={self.id}, value={self.value})>"