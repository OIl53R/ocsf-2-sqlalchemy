"""Generated primitive_array model: ocsf_group_privileges.
...
class OcsfOcsfGroupPrivileges(OcsfBase):
    """Association table for group.privileges.

    Stores primitive array values in a normalized one-to-many relationship.
    """
    __tablename__ = "ocsf_group_privileges"
    ...
    value: Mapped[Optional[str]] = mapped_column(
        Text,
        nullable=True,
        comment="Values for group.privileges",
    )
    ...
    group: Mapped["OcsfGroup"] = relationship(
        "OcsfGroup",
        back_populates="privileges",
    )

    def __repr__(self) -> str:
        return f"<OcsfOcsfGroupPrivileges(id={self.id}, value={self.value})>"